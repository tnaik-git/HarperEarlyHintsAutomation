...
def main():
    ...
    "--skip-gtm",
    action="store_true",
    help="Skip GTM workflow and reuse existing GTM domain property"
    )

    parser.add_argument(
    "--github-bundle",
    action="store_true",
    help="Pull EdgeWorker main.js and bundle.json from GitHub instead of local files"
    )
    ...
    skipGtm = args.skip_gtm or config.get("skipGtm", False) if 'config' in dir() else args.skip_gtm
    ...
    # 1. GTM WORKFLOW
    # ============================================================
    skip_gtm = args.skip_gtm or config.get("skipGtm", False)

    if skip_gtm:
        existing_domain = config.get("existingGtmDomainPropertyName") or config.get("gtmDomain")
